...
describe('HCSF Precision Calculations', () => {
  ...
  it('should use TMI-based estimation when revenus_activite is 0 or missing', () => {
    ...
    const result = analyserHcsf(
      baseInput as unknown as CalculationInput,
      fakeFinancement,
      1000,
      mockConfig
    );
  ...
  it('should use exact revenus_activite when provided', () => {
    ...
    const result = analyserHcsf(
      inputWithIncome as unknown as CalculationInput,
      fakeFinancement,
      1000,
      mockConfig
    );
...
describe('estimerRevenusDepuisTmi — couverture par tranche', () => {
  ...
  it('TMI 11 → estimation 1 800 €/mois → taux ≈ 65%', () => {
    ...
    expect(result.taux_endettement).toBeCloseTo(65.0, 1);
  ...
  it('TMI 41 → estimation 7 000 €/mois → taux ≈ 21.1%', () => {
    ...
    expect(result.taux_endettement).toBeCloseTo(21.1, 1);
...
describe('REC-02 : calculerCapaciteResiduelle avec config custom', () => {
  ...
  it('taux 5%/15 ans → capacité inférieure à celle avec 3.5%/20 ans', () => {
    const configHautTaux = {
      ...mockConfig,
      hcsfTauxReferenceCapacite: 0.05,
      hcsfDureeCapaciteResiduelleAnnees: 15,
    };
    const resultDefault = analyserHcsf(
      baseInput as unknown as CalculationInput,
      fakeFinancement,
      2000,
      mockConfig
    );
    const resultHautTaux = analyserHcsf(
      baseInput as unknown as CalculationInput,
      fakeFinancement,
      2000,
      configHautTaux
    ...
    expect(resultHautTaux.capacite_emprunt_residuelle).toBeLessThan(
      resultDefault.capacite_emprunt_residuelle
    );
  });
});
