...
describe('AUDIT-104 : Amortissement par composants', () => {
  ...
  describe('Mode simplifie vs composants en LMNP Reel', () => {
    ...
    it('mode composants : amortissement plus eleve', () => {
      ...
      expect(
        result.alertes.some((a) => a.includes('8387') || a.includes('8 387') || a.includes('8386'))
      ).toBe(true);
    });
  ...
  describe('Mode composants en SCI IS', () => {
    it("utilise l'amortissement par composants en SCI IS", () => {
      ...
      const resultSimp = calculerFiscaliteSciIs(
        12200,
        200000,
        mockConfig,
        5000,
      ...
      const resultComp = calculerFiscaliteSciIs(
        12200,
        200000,
        mockConfig,
        5000,
...
describe('genererTableauAmortissementFiscal', () => {
  ...
  it('retourne un tableau LMNP réel avec economieImpotEstimee basée sur TMI + PS', () => {
    ...
    expect(result!.totaux.economieImpotEstimee).toBeCloseTo(
      result!.totaux.totalDeductible * (0.3 + mockConfig.tauxPsRevenusBicLmnp),
      -2
    );
  ...
  it("LMNP réel : l'amortissement excédentaire est reporté (cap à 0 base imposable)", () => {
    ...
    expect(result!.lignes[0].amortissementDeductible).toBeLessThan(
      result!.lignes[0].amortissementTotal
    );
  });
});
