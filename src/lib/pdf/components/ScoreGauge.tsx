...
function AjustementBarPdf({ label, value, range }: { label: string; value: number; range: [number, number] }) {
    const [min, max] = range;
            ...
            <View style={{ width: '90%', marginTop: 10 }}>
                <View style={{ flexDirection: 'row', height: 8, borderRadius: 4, overflow: 'hidden' }}>
                    {LEGEND_SEGMENTS.map((seg) => (
                        <View key={seg.label} style={{ width: seg.width, backgroundColor: seg.color, height: 8 }} />
                    ))}
                </View>
                ...
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 4 }}>
                    {LEGEND_SEGMENTS.map((seg) => (
                        <Text key={seg.label} style={gaugeStyles.legendLabel}>{seg.label}</Text>
                    ))}
                </View>
            </View>

            {/* Verdict badge */}
            <View style={[gaugeStyles.verdictBadge, { backgroundColor: verdictStyle.bg, borderColor: verdictStyle.border }]}>
...
export function ScoreGauge({ score, label = 'Indice de Performance', scoreDetail }: ScoreGaugeProps) {
    const scoreColor = getScoreColor(score);
    ...
    const ajustementKeys = ['cashflow', 'rentabilite', 'hcsf', 'dpe', 'ratio_prix_loyer', 'reste_a_vivre'] as const;
        ...
        <View style={gaugeStyles.container}>
            ...
            <View style={gaugeStyles.rightCol}>
                ...
                <View style={[gaugeStyles.decompRow, { marginBottom: 2 }]}>
                    <Text style={[gaugeStyles.decompLabel, { color: colors.textMain, fontWeight: 'bold' }]}>Base initiale</Text>
                    <View style={{ flex: 1 }} />
                    <Text style={[gaugeStyles.decompValue, { color: colors.textMain }]}>{scoreDetail.base}</Text>
                </View>
