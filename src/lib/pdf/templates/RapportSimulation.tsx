...
const DISCLAIMER =
  "Ce document est fourni à titre informatif uniquement et ne constitue pas un conseil financier, fiscal ou juridique. Les résultats sont des estimations basées sur les données saisies. Consultez un professionnel avant toute décision d'investissement.";

export function RapportSimulation({
  formData,
  resultats,
  generatedAt = new Date(),
}: RapportSimulationProps) {
  const { bien, financement, exploitation, structure } = formData;
  const { rentabilite, cashflow, fiscalite, hcsf, synthese, projections, comparaisonFiscalite } =
    resultats;
    ...
    <Document>
      ...
      <Page size="A4" style={styles.page}>
        ...
        {projections && projections.totaux && (
          <FinancialTable rentabilite={rentabilite} cashflow={cashflow} fiscalite={fiscalite} />
        )}
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View wrap={false}>
          ...
          <View style={[styles.card, { borderLeftWidth: 4, borderLeftColor: colors.primary }]}>
            <Text
              style={{ fontSize: 11, fontWeight: 'bold', color: colors.primary, marginBottom: 5 }}
            >
              Avis de l&apos;analyste
            </Text>
        ...
        <View
          style={{ marginTop: 20, padding: 15, backgroundColor: colors.surface, borderRadius: 4 }}
        >
          <Text
            style={{ fontSize: 9, fontWeight: 'bold', color: colors.textMuted, marginBottom: 5 }}
          >
            AVERTISSEMENT
          </Text>
