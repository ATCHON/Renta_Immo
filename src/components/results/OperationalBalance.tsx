...
export function OperationalBalance({
  ...
  return (
    <Card className="h-full">
      <CardHeader title="Bilan d'Exploitation" description="Flux de trésorerie mensuel moyen" />
        ...
        <div className="pt-4 border-t border-sand/50">
          <div className="flex items-center justify-between">
            ...
            >
              {isPositive
                ? 'Projet Sain'
                : `${((Math.abs(cashflow.mensuel) / (loyerMensuel || 1)) * 100).toFixed(0)}% du loyer`}
            </div>
          </div>
        </div>
      </CardContent>
