...
export function StepOptions({ onSubmit, onPrev, isLoading }: StepOptionsProps) {
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-on-surface">Options</h2>
      ...
      {watchedValues.envoyer_email && (
        <div className="pl-4 border-l-2 border-primary/30 transition-all animate-in fade-in slide-in-from-top-2">
          <Input
            label="Adresse email"
            type="email"
      ...
      {/* Récapitulatif */}
      <div className="bg-secondary-fixed/40 rounded-2xl p-5">
        <h3 className="font-bold text-on-surface mb-3">Récapitulatif</h3>
        <ul className="text-sm text-on-surface-variant space-y-2">
          <li className="flex items-center gap-2">
            <span className="text-primary">✓</span> Calcul de rentabilité (brute, nette,
            nette-nette)
          </li>
          <li className="flex items-center gap-2">
            <span className="text-primary">✓</span> Analyse du cashflow mensuel et annuel
          </li>
          <li className="flex items-center gap-2">
            <span className="text-primary">✓</span> Simulation fiscale selon votre structure
          </li>
          <li className="flex items-center gap-2">
            <span className="text-primary">✓</span> Vérification conformité HCSF
          </li>
          <li className="flex items-center gap-2">
          ...
          {watchedValues.generer_pdf && (
            <li className="flex items-center gap-2">
              <span className="text-primary">✓</span> Génération du PDF récapitulatif
            </li>
          )}
          {watchedValues.envoyer_email && (
            <li className="flex items-center gap-2">
              <span className="text-primary">✓</span> Envoi par email à{' '}
              {watchedValues.email || '...'}
            </li>
          )}
        </ul>
    ...
    >
      <div
        className={cn(
          ...
          checked ? 'bg-primary border-primary' : 'bg-surface-container border-outline-variant'
        )}
      >
        {checked && (
          <svg
