            ...
            <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-slate-200">
                    <thead className="bg-slate-50">
                        <tr>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Paramètre</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Clef</th>
                            <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider text-right">Valeur actuel</th>
                            <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase tracking-wider">Unité</th>
                    ...
                    <tbody className="divide-y divide-slate-200">
                        {loading ? (
                            ...
                            <tr><td colSpan={5} className="px-6 py-12 text-center text-slate-400">Aucun paramètre trouvé pour ce bloc.</td></tr>
                        ) : (
                            params.map((param) => (
                                <tr key={param.id} className="hover:bg-slate-50 transition-colors">
                                    <td className="px-6 py-4">
                                        ...
                                        {param.description && <div className="text-xs text-slate-500 max-w-xs">{param.description}</div>}
                                        {param.isTemporary && (
                                    ...
                                    <td className="px-6 py-4 text-right font-semibold text-forest">
                                        {param.unite === 'pourcentage' ? `${(param.valeur * 100).toFixed(2)}%` : param.valeur}
                                    </td>
