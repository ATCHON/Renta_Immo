    ...
    return (
        <div className="mt-6 space-y-4">
            <h3 className="font-semibold text-slate-900 flex justify-between items-center">
                Impact simulé
                <button onClick={runSimulation} className="text-xs text-forest hover:underline">Rafraîchir</button>
            </h3>
            ...
            {changedScenarios.length === 0 ? (
                ...
                <div className="space-y-3">
                    {changedScenarios.map((scenario) => (
                        <div key={scenario.scenarioId} className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-sm">
                            <div className="font-medium text-slate-700 mb-2">{scenario.label}</div>
                            <div className="grid grid-cols-2 gap-2">
                                <MetricDiff label="Rentabilité" delta={scenario.metrics.rentabilite.delta} unit="%" isPercent />
...
function MetricDiff({ label, delta, unit, isPercent, inverse }: { label: string, delta: number, unit: string, isPercent?: boolean, inverse?: boolean }) {
    ...
    return (
        <div className="flex justify-between">
            ...
            <span className={`font-mono font-medium ${colorClass}`}>
                {sign}{isPercent ? delta.toFixed(2) : Math.round(delta)}{unit}
            </span>
        </div>
    );
}
