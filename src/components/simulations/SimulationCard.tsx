    ...
    <div className="group bg-white rounded-[2.5rem] shadow-sm hover:shadow-2xl hover:shadow-slate-200 border border-slate-100 p-8 transition-all duration-300 relative">
      <div className="flex justify-between items-start mb-6">
        <div
          className={`w-14 h-14 rounded-2xl flex items-center justify-center font-bold text-lg ${
            (simulation.score_global || 0) >= 70
              ? 'bg-emerald-50 text-emerald-600'
              : (simulation.score_global || 0) >= 40
      ...
      <Link href={`/simulations/${simulation.id}`} className="block">
        ...
        <div className="grid grid-cols-2 gap-4 pt-6 border-t border-slate-50">
          <div>
            <p className="text-[10px] uppercase tracking-wider font-black text-slate-300 mb-1">
              Rentabilité
            </p>
          ...
          <div>
            ...
            <p
              className={`font-bold ${(simulation.cashflow_mensuel || 0) >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}
            >
              {(simulation.cashflow_mensuel || 0) >= 0 ? '+' : ''}
              {formatCurrency(simulation.cashflow_mensuel || 0)}
            </p>
          </div>
        </div>
      </Link>
