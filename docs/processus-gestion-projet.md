...
3.  **Création d'une nouvelle Story** :
    - **Dans Notion d'abord** : Créer l'entrée dans la _DB Stories_ (attribuer un ID, l'lier à une Epic, définir la priorité/complexité).
...
- **🟥 Bloc 3 — Bugs Critiques** : Correspond un affichage filtré de la _DB Bugs_ (Priorité = Critique, Statut ≠ Résolu). **Ces anomalies bloquantes ou majeures en production/développement ont la priorité absolue sur les Stories de fonctionnalités.**
